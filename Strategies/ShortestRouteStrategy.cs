// Arquivo: ShortestRouteStrategy.cs
// Implementação da estratégia de rota mais curta
...
{
    // Estratégia para calcular a rota mais curta
    public class ShortestRouteStrategy : RouteStrategy
    {
        ...
        {
            Console.WriteLine("Calculando rota mais CURTA...");
            
            // Simula o cálculo de rota baseado na minimização da distância
            // Em um sistema real, isso usaria dados de mapas e algoritmos reais
            
            Random rand = new Random();
            double distance = SimulateDistance(origin, destination);
            // Rota mais curta usa a menor distância por definição
            distance *= 0.9; // 10% mais curta que a rota padrão
            ...
            // Rota mais curta pode usar ruas menores, o que aumenta o tempo
            estimatedTime = (int)(estimatedTime * 1.1); // 10% de tempo extra potencialmente
            ...
            route.Instructions.Add("Siga pela rota com menor distância total");
            route.Instructions.Add($"Distância total: {distance:F1} km");
